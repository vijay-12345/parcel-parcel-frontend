...
let tablesjson = {
    ...
    "FaturasLiquidar": {
        ...
        actionbuttons: { File: "Montante", Invoice_File: "Fatura" , Arrow: true , DropdownRevenue: true },
    ...
    "AllServicesDetails": {
        ...
        actionbuttons: { Invoice_File: "Fatura", Arrow: true },
        popUpTabName: "Faturas pendentes",
        listApilink: "Revenue/GetInvoiceList",
        requestMethod: "post",
        notAction: false,
    ...
    "gerfip": {
        ...
        actionbuttons: { detailPopUp: true, AllFile: true },
        popUpTabName: "Faturas pendentes",
        listApilink: "Revenue/GetInvoiceList",
        requestMethod: "post",
        notAction: false,
    ...
    "general_account": {
        columnkeyMap: {
            "entityObject":"Nome da entidade",
            "trans_date": "Data da transação",
            "trans_amount": "valor da transação",
            "balance": "Saldo",
            "comment":"Comente"
        },
        actionbuttons: { File: true },
        popUpTabName: "Faturas pendentes",
        listApilink: "Revenue/GeneralAccount/GetAll",
        requestMethod: "post",
        notAction: true,
        DocumentGERFIP: "",
        noDownloadButton: true,
...
class RevenueTables extends Component {
        ...
        if (res) {
            res.map((_row, index) => {
                ...
                Object.keys(columnkeyMap).map(function (key) {
                    if (key === "n doc") {
                    ...
                    }else if (key === "entityObject") {
                        _row[columnkeyMap[key]] =_row.entity_id ?  _row.entityObject.nome:""
                    } else if (key === "Sinal") {
                        ...
                        _row[columnkeyMap[key]] = services.money.format(_row["trans_amount"]) ;
                    } else if (key === "balance") {
                        _row[columnkeyMap[key]] =  services.money.format(_row["balance"]);
                    }
    ...
    createTabel = () => {
        console.log("LIST DATA" ,this.state.filterData);
    ...
    ActionPerform = (action, row) => {
        if(this.props.ActionPerform){
            this.props.ActionPerform(action, row)
        }else{
            let popUpForm = null;
