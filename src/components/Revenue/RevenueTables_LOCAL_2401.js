...
class RevenueTables extends Component {
    ...
    getListData = async () => {
        ...
        if (res) {
            res.map((_row, index) => {
                if(_row.document_url){
                   _row.document_url = "/api/"+_row.document_url.replace("file:///C:/DevelopmentIVPD","");
                }
                if(_row.invoice_url){
                    // if(_row.is_doc_generated===1)
                    _row.invoice_url = "/api/"+_row.invoice_url.replace("file:///C:/DevelopmentIVPD","");
                  //  else
                //   _row.invoice_url = null 
                }
